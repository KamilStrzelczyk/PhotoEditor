...
public class ImageCropperViewModel {
    ...
    @Inject
    public ImageCropperViewModel(PhotoSourceRepository photoSourceRepository, SetTrimOnImageUseCase setTrimOnImage) {
        this.setTrimOnImage = setTrimOnImage;
        this.photoSourceRepository = photoSourceRepository;
        Disposable frameInfoSubscription = photoSourceRepository.getFrameInfo().subscribe(newFrameInfo -> {
